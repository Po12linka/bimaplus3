...
print("BIM A+")
print("-" * 46)
print("Polina Sherstobitova")
print("-" * 46)
print("BIMA+_Assignment#1")
print("-" * 46)
...
# The lists that store all entered coordinates. They're empty at start.
xs = []
ys = []
...
for i in range(n):
   ...
   Ix=Ix-(xs[i+1]-xs[i])*(ys[i+1]**3+ys[i+1]**2*ys[i]+ys[i+1]*ys[i]**2+ys[i]**3)/12
   Iy=Iy+(ys[i+1]-ys[i])*(xs[i+1]**3+xs[i+1]**2*xs[i]+xs[i+1]*xs[i]**2+xs[i]**3)/12
   Ixy=Ixy-(ys[i+1]-ys[i])*(ys[i+1]*(3*xs[i+1]**2+2*xs[i+1]*xs[i]+xs[i]**2)+ys[i]*(3*xs[i]**2+2*xs[i+1]*xs[i]+xs[i+1]**2))/24
